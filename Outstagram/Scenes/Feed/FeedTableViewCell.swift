...
final class FeedTableViewCell: UITableViewCell {
    ...
    private lazy var contentsLabel: UILabel = {
        ...
        label.text = "이 편지는 영국에서 시작되어, 글을 본 10일 이내에 본인의 코믹한 사진을 포스팅 하지 않으면, 인스타그램 세상에서 행복할 수 없을 것이다. 이 편지의 내용을 무시하고, 지시사항을 수행하지 않은 김아무개는 그 이후로 좋아요를 받지 못했으며 결국 팔로워도 0이 되었다."
        return label
    }()
    ...
    func setup(){
        ...
        contentsLabel.snp.makeConstraints{
            $0.leading.equalTo(likeButton.snp.leading)
            $0.trailing.equalTo(bookmarkButton.snp.trailing)
            $0.top.equalTo(likedCountLabel.snp.bottom).offset(8)
        }
